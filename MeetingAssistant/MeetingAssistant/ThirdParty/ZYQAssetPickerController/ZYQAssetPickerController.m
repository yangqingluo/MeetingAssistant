...
#define kThumbnailLength    ([UIScreen mainScreen].bounds.size.width / numberOfRow)
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    ...
    if (_indexPathsForSelectedItems.count >= vc.maximumNumberOfSelection) {
        if ([_indexPathsForSelectedItems indexOfObject:asset] == NSNotFound) {
            
            [vc showHint:[NSString stringWithFormat:@"超过最大选择数目"]];
...
- (void)finishPickingAssets:(id)sender
{
    ...
    if (_indexPathsForSelectedItems.count < picker.minimumNumberOfSelection) {
        if (picker.delegate!=nil&&[picker.delegate respondsToSelector:@selector(assetPickerControllerDidMinimum:)]) {
            [picker.delegate assetPickerControllerDidMinimum:picker];
        }
    }
